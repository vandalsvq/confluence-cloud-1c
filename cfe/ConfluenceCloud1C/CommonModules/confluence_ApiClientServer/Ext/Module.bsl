...
Функция get_pdf_download_url(знач Настройки, знач СтраницаИД) Экспорт
	...
	// 1. Вызываем запуск задания для формирования PDF
	//		.../wiki/spaces/flyingpdf/pdfpageexport.action?pageId=(ИдентификаторСтраницы)
	// 2. В полученном ответе находим идентификатор задания
	// 3. По идентификатору задания регулярно опрашиваем статус выполнения
	// 4. После завершения по адресу файла возвращаем публичную ссылку для скачивания
		
	ТекстЗапроса = СтрШаблон("wiki/spaces/flyingpdf/pdfpageexport.action?pageId=%1", СтраницаИД);
	...
	ОтветHTTP = СоединениеHTTP.Получить(ЗапросHTTP);
	Если ОтветHTTP.КодСостояния <> 403 Тогда
...
Функция ПолучитьЗаголовки(знач Настройки, знач ТипКонтента = "application/json")
