...
Функция get_space_content(знач Настройки, знач КлючПространства, знач ПараметрыЗапроса = Неопределено) Экспорт
	ПолучитьКонтент = СвойствоСтруктуры(ПараметрыЗапроса, "ПолучитьКонтент", Ложь);
	
	СтрокаПараметры = ПараметрыЗапроса_get_space_content(ПараметрыЗапроса);
	СтрокаПараметры = ДополнитьПараметрыЗапроса(СтрокаПараметры, "expand", ПолучитьНастройкуСодержимогоКонтента(ПолучитьКонтент));
...
//		ПолучитьКонтент	- Булево - включить в состав Структуры content, значение свойства body_storage
// 
Функция get_space_content_params(знач Начало = 0, знач Количество = 0) Экспорт
	...
	Структура.Вставить("ПолучитьКонтент", Ложь);
...
Функция ПолучитьНастройкуСодержимогоКонтента(знач ВключитьСодержимое = Ложь)
	Возврат "childTypes.page,description.view,version,ancestors,space"
		+ ?(ВключитьСодержимое, ",body.storage", "");
КонецФункции
