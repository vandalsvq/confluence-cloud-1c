...
Функция get_binarydata_file(знач Настройки, знач СтраницаИД, знач ВложениеИД) Экспорт
	...
	Если ОтветHTTP.КодСостояния <> 200 Тогда
		Возврат Структура_error(ОтветHTTP, НСтр("ru='Скачивание файла.'"));
	КонецЕсли;
...
// Возвращает двоичные данные экспортированными данными страницы в формате PDF
//
// Параметры:
//	Настройки	- Структура - см. get_connection_settings
//	СтраницаИД	- Строка - идентификатор страницы
...
Функция get_page_as_pdf(знач Настройки, знач СтраницаИД) Экспорт
	Результат = get_pdf_download_url(Настройки, СтраницаИД);
	Если НЕ ТипЗнч(Результат) = Тип("Строка") Тогда
		Возврат Результат;
	КонецЕсли;
	...
	СоединениеHTTP	= Новый HTTPСоединение(СтруктураАдреса.ИмяСервера,,,,,, ЗащищенноеСоединение);
	ЗапросHTTP		= Новый HTTPЗапрос(СтруктураАдреса.ПутьНаСервере, Заголовки);
...
// Возвращает двоичные данные экспортированными данными страницы в формате DOC
//
// Параметры:
//	Настройки	- Структура - см. get_connection_settings
//	СтраницаИД	- Строка - идентификатор страницы
...
Функция get_page_as_word(знач Настройки, знач СтраницаИД) Экспорт
	...
	ОтветHTTP = СоединениеHTTP.Получить(ЗапросHTTP);
	Если ОтветHTTP.КодСостояния <> 200 Тогда
		Возврат Структура_error(ОтветHTTP, НСтр("ru='Получение ссылки для скачивания Word'"));
	КонецЕсли;
...
// Параметры:
//	Настройки		- Структура - см. get_connection_settings
//	ТекстЗапроса	- Строка - текст запроса /wiki/download/...
//
// Возвращаемое значение:
//   Строка
//
Функция get_pdf_download_url(знач Настройки, знач СтраницаИД) Экспорт
	...
	//		.../wiki/spaces/flyingpdf/pdfpageexport.action?pageId=(ИдентификаторСтраницы)
	...
	ТекстЗапроса = СтрШаблон("wiki/spaces/flyingpdf/pdfpageexport.action?pageId=%1&atl_token=%2",
		СтраницаИД,
		ДанныеСессии.token);
	
	ЗаголовкиHTTP	= ПолучитьЗаголовки(Настройки, "text/html;charset=UTF-8", ДанныеСессии.cookie);
	ЗапросHTTP		= Новый HTTPЗапрос(ТекстЗапроса, ЗаголовкиHTTP);
	СоединениеHTTP	= ПолучитьСоединениеHTTP(Настройки);

	ОтветHTTP = СоединениеHTTP.Получить(ЗапросHTTP);
	Если ОтветHTTP.КодСостояния <> 200 Тогда
		Возврат Структура_error(ОтветHTTP, НСтр("ru='Получение ссылки для скачивания PDF'"));
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ЗадачаИД) Тогда
		Возврат Структура_error(ОтветHTTP, НСтр("ru='Получение номера задания для экспорта в PDF'"));
	КонецЕсли;
	
	ТекстПроверки = СтрШаблон("wiki/runningtaskxml.action?taskId=%1", ЗадачаИД);
	...
	Пока ВыполнятьПроверку Цикл
		...
		ЧтениеXML.ОткрытьПоток(ТелоПотокДанных);
		
		ОбъектФабрика	= Новый ФабрикаXDTO;
		ОбъектXDTO	= ОбъектФабрика.ПрочитатьXML(ЧтениеXML);
		ОбъектФабрика	= Неопределено;
		ЧтениеXML = Неопределено;
		
		Завершена	= (ОбъектXDTO.Получить("isComplete") = "true");
		Успешно		= (ОбъектXDTO.Получить("isSuccessful") = "true");
		ПрошлоВремя	= ОбъектXDTO.Получить("elapsed");
		Процент		= ОбъектXDTO.Получить("percentComplete");
		...
		Если Завершена И Успешно Тогда
			ТекстСообщения = ОбъектXDTO.Получить("currentStatus");
			
			Разделитель1 = СтрНайти(ТекстСообщения, "<a class=""space-export-download-path""");
			Если Разделитель1 = 0 Тогда
				Прервать;
			КонецЕсли;
			
			Разделитель2 = СтрНайти(ТекстСообщения, "href=""",, Разделитель1 + 1);
			Если Разделитель2 = 0 Тогда
				Прервать;
			КонецЕсли;
			
			Разделитель3 = СтрНайти(ТекстСообщения, """>",, Разделитель2 + 1);
			Если Разделитель3 = 0 Тогда
				Прервать;
			КонецЕсли;
			
			АдресСсылки = Сред(ТекстСообщения, Разделитель2 + 6, Разделитель3 - Разделитель2 - 6);
		КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(АдресСсылки) Тогда
		Возврат Структура_error(ОтветHTTP, НСтр("ru='Ошибка определения адреса размещения файла PDF'"));
	КонецЕсли;
...
//   Строка
//
Функция get_data_publiclink(знач Настройки, знач ТекстЗапроса) Экспорт
	#Область ЗапросHTTP
	ЗаголовкиHTTP	= ПолучитьЗаголовки(Настройки);
	ЗапросHTTP		= Новый HTTPЗапрос(ТекстЗапроса, ЗаголовкиHTTP);
	СоединениеHTTP	= ПолучитьСоединениеHTTP(Настройки);
	...
	Если ОтветHTTP.КодСостояния <> 302 Тогда
		Возврат Структура_error(ОтветHTTP, НСтр("ru='Проверка наличия данных для скачивания.'"));
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(АдресФайла) Тогда
		...
		Возврат Структура_error(ОтветHTTP, ТекстОписания);
	КонецЕсли;
...
Функция get_long_url(знач Настройки, знач КороткийURL) Экспорт
	...
	ОтветHTTP = СоединениеHTTP.Получить(ЗапросHTTP);
	Если ОтветHTTP.КодСостояния <> 302 Тогда
		Возврат Структура_error(ОтветHTTP, НСтр("ru='Определение полного адреса ссылки.'"));
	КонецЕсли;
...
Функция get_session_data(знач Настройки) Экспорт
	...
	ОтветHTTP = СоединениеHTTP.Получить(ЗапросHTTP);
	Если ОтветHTTP.КодСостояния <> 200 Тогда
		Возврат Структура_error(ОтветHTTP, НСтр("ru='Получение данных cookie для сессии.'"));
	КонецЕсли;
...
Функция get_all_pages_from_space(знач Настройки, знач КлючПространства, знач ПараметрыЗапроса = Неопределено) Экспорт
	...
	ОтветHTTP = СоединениеHTTP.Получить(ЗапросHTTP);
	Если ОтветHTTP.КодСостояния <> 200 Тогда
		Возврат Структура_error(ОтветHTTP, НСтр("ru='Получение всех страниц раздела.'"));
	КонецЕсли;
...
Функция get_attachments_from_content(знач Настройки, знач СтраницаИД, знач ПараметрыЗапроса = Неопределено) Экспорт
	...
	ДанныеОтвета = СоответствиеИзОтвета(ОтветHTTP);
	Возврат Структура_attachments(ДанныеОтвета);
КонецФункции
...
//	ПолеПоиска	- см. cql_filterItem
//	Условие		- см. cql_filterItem
//	Значение	- см. cql_filterItem
// 
Процедура cql_addFilterItem(Коллекция, знач ПолеПоиска, знач Условие, знач Значение) Экспорт
...
//
Функция Структура_attachment(знач Соответствие)
	Результат = Новый Структура;
...
Функция Структура_content(знач Соответствие, знач Настройки)
	...
	Результат.Вставить("Ссылки"				, Структура_links(ДанныеСсылки));
	Результат.Вставить("Вложения"			, Структура_attachments(ДанныеВложений));
...
Функция Структура_contentchildren(знач Соответствие, знач Настройки)
	...
	Результат.Вставить("Вложения"			, Структура_attachment(ДанныеВложений));
...
//		РазмерВыборки	- Число - количество строк в выборке
//
Функция Структура_attachments(знач Соответствие)
	ДанныеРезультат = СвойствоСоответствия(Соответствие, "results", Новый Массив);
	
	МассивРезультат = Новый Массив;
	Для Каждого СоответствиеРезультат Из ДанныеРезультат Цикл
		СтруктураВложенгие = Структура_attachment(СоответствиеРезультат);
...
Функция ПолучитьЗаголовки(знач Настройки, знач ТипКонтента = "application/json", знач СтрCookie = "")
	Заголовки = Новый Соответствие;
	...
	Если НЕ ПустаяСтрока(СтрCookie) Тогда
		Заголовки.Вставить("cookie", СтрCookie);
	КонецЕсли;
