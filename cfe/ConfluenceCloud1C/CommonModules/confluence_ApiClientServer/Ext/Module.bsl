...
Функция get_binarydata_file(знач Настройки, знач СтраницаИД, знач ВложениеИД) Экспорт
	Результат = get_data_publiclink(Настройки, СтраницаИД, ВложениеИД);
...
// Проверяет наличие присоединенного файла и возвращает адрес к файлу для скачивания
//
// Параметры:
//	Настройки		- Структура - см. get_connection_settings
//	ТекстЗапроса	- Строка - текст запроса /wiki/download/...
...
//   Строка
//
Функция get_data_publiclink(знач Настройки, знач СтраницаИД, знач ВложениеИД) Экспорт
	...
	Если ОтветHTTP.КодСостояния <> 302 Тогда
		Возврат Структура_error(ОтветHTTP, НСтр("ru='Проверка наличия файла.'"));
	КонецЕсли;
	
	АдресФайла = СвойствоСоответствия(ОтветHTTP.Заголовки, "location", "");
	Если НЕ ЗначениеЗаполнено(АдресФайла) Тогда
		Возврат Структура_error(ОтветHTTP, НСтр("ru='Определение адреса для скачивания файла.'"));
	КонецЕсли;
