...
Функция get_space_xml_download_url(знач Настройки, знач КлючПространства) Экспорт
	...
	Если ОтветHTTP.КодСостояния <> 200 Тогда
		Возврат Структура_error(ОтветHTTP, НСтр("ru='Получение ссылки для скачивания ZIP архива XML'"));
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ТекстПроверки) Тогда
		Возврат Структура_error(ОтветHTTP, НСтр("ru='Получение ссылки длительного задания для экспорта в XML'"));
	КонецЕсли;
	...
	Пока ВыполнятьПроверку Цикл
		ОтветHTTP = СоединениеHTTP.Получить(ЗапросHTTP);
		Если ОтветHTTP.КодСостояния <> 200 Тогда
			Возврат Структура_error(ОтветHTTP, НСтр("ru='Получение ссылки для скачивания zip архива с данными XML.'"));
		КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(АдресСсылки) Тогда
		Возврат Структура_error(ОтветHTTP, НСтр("ru='Ошибка определения адреса размещения файла zip архива с данными XML'"));
	КонецЕсли;
...
Функция get_page_labels(знач Настройки, знач СтраницаИД, знач ПараметрыЗапроса = Неопределено) Экспорт
	...
	ОтветHTTP = СоединениеHTTP.Получить(ЗапросHTTP);
	Если ОтветHTTP.КодСостояния <> 200 Тогда
		Возврат Структура_error(ОтветHTTP, НСтр("ru='Получение меток по странице.'"));
	КонецЕсли;
	
	ДанныеОтвета = СоответствиеИзОтвета(ОтветHTTP);
	Возврат Структура_labelarray(ДанныеОтвета);
КонецФункции
...
Функция Структура_content(знач Соответствие, знач Настройки)
	...
	Результат.Вставить("Метки"				, Структура_labelarray(ДанныеМеток));
...
//		Метка			- Строка
//
Функция Структура_label(знач Соответствие)
	Результат = Новый Структура;
	Результат.Вставить("ИсходныеДанные"	, Соответствие);
...
//		РазмерВыборки	- Число
//
Функция Структура_labelarray(знач Соответствие)
	ДанныеРезультат = СвойствоСоответствия(Соответствие, "results ", Новый Массив);
	
	МассивРезультат = Новый Массив;
	Для Каждого СоответствиеМетка Из ДанныеРезультат Цикл
		СтруктураМетка = Структура_label(СоответствиеМетка);
