...
Процедура УстановитьУсловноеОформлениеРеквизита(Форма, Обработано, ИмяРеквизита)
	...
	Если ИмяРеквизита = "РеквизитыАнонимныйДоступ" ИЛИ ПустаяСтрока(ИмяРеквизита) Тогда
		УстановитьУсловноеОформлениеРеквизита(Форма, Обработано, "КомандаЭкспортПространстваВXML");
		УстановитьУсловноеОформлениеРеквизита(Форма, Обработано, "КомандаЭкспортПространстваВPDF");
	КонецЕсли;
	...
	Если ИмяРеквизита = "КомандаЭкспортПространстваВXML" Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ФормаЭкспортПространстваВXML", "Видимость", Форма.ЭтоАдминистратор И НЕ Форма.ЭтоАнонимныйДоступ);
	КонецЕсли;
	
	Если ИмяРеквизита = "КомандаЭкспортПространстваВPDF" Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ФормаЭкспортПространстваВPDF", "Видимость", Форма.ЭтоАдминистратор И НЕ Форма.ЭтоАнонимныйДоступ);
	КонецЕсли;
...
Процедура КлючПространстваПриИзменении(Элемент)
	...
	ДанныеПространства = ПолучитьДанныеСтраницыПространства(ЭтотОбъект);
	ВыполнитьПереходНаСтраницу(ДанныеПространства.СтраницаИдентификатор, ДанныеПространства.Наименование);
...
Процедура ПолеHTMLДокументСформирован(Элемент)
	...
	ДанныеПространства = ПолучитьДанныеСтраницыПространства(ЭтотОбъект);
	Если НЕ ДанныеПространства = Неопределено И ДанныеПространства.АнонимныйДоступ Тогда
		ЭтотОбъект.КартинкиОбработаны = Истина;
		ОбновитьПиктограммуЗагрузкиСтраницыНаКлиенте(Истина);
	КонецЕсли;
	
	Если НЕ ЭтотОбъект.КартинкиОбработаны Тогда
		ОбработатьКартинкиСтраницыНаКлиенте();
	КонецЕсли;
...
Процедура ДеревоСтраницПередРазворачиванием(Элемент, Строка, Отказ)
	...
	ДанныеСтроки = Новый Структура("СтраницаИдентификатор,СсылкаПросмотр",
		ТекущиеДанные.Идентификатор,
		ТекущиеДанные.СсылкаПросмотр);
		
	ЗаполнитьПодчиненныеСтраницыДереваНаКлиенте(ТекущиеДанные, ДанныеСтроки);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.ЭтоАдминистратор		= Пользователи.РолиДоступны("confluence_ПолныеПрава");
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(Объект.КлючПространства) Тогда
		ДанныеПространства = ПолучитьДанныеСтраницыПространства(ЭтотОбъект);
		ВыполнитьПереходНаСтраницу(ДанныеПространства.СтраницаИдентификатор, ДанныеПространства.Наименование);
	КонецЕсли;
...
Процедура ОткрытьСсылку(Команда)
	Если НЕ ЗначениеЗаполнено(ЭтотОбъект.ТекущаяСтраницаАдресСтраницы) Тогда
		Возврат;
	КонецЕсли;
	
	ОткрытьВнешнююСсылку(ЭтотОбъект.ТекущаяСтраницаАдресСтраницы);
КонецПроцедуры
...
Асинх Процедура СохранитьКакPDF(Команда)
	...
	ИмяФайла = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(ЭтотОбъект.ТекущаяСтраницаПредставление, "");
...
&НаКлиенте
Асинх Процедура ЭкспортПространстваВXML(Команда)
	Если НЕ ЗначениеЗаполнено(Объект.КлючПространства) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Текущее пространство не определено'"), 30);
		Возврат;
	КонецЕсли;
	
	ПутьНаКлиенте = Ждать confluence_FileSystemAsync.ВыбратьФайл(РежимДиалогаВыбораФайла.Сохранение,
		НСтр("ru='Архив ZIP'") + "(*.zip)|*.zip",
		НСтр("ru='Укажите путь для сохранения файла архива'"),
		Объект.КлючПространства + ".zip");
	...
	ЭкспортПространстваВXMLНаКлиенте(ПутьНаКлиенте);
КонецПроцедуры

&НаКлиенте
Асинх Процедура ЭкспортПространстваВPDF(Команда)
	Если НЕ ЗначениеЗаполнено(ЭтотОбъект.ТекущаяСтраницаИдентификатор) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Откройте статью базы знаний для сохранения'"), 30);
		Возврат;
	КонецЕсли;
	...
	ЭкспортПространстваPDFНаКлиенте(ПутьНаКлиенте);
КонецПроцедуры
...
&НаКлиенте
Процедура ВыводитьСостояниеЗагрузкиКартинок(Команда)
	Элементы.ФормаВыводитьСостояниеЗагрузкиКартинок.Пометка = НЕ Элементы.ФормаВыводитьСостояниеЗагрузкиКартинок.Пометка;
КонецПроцедуры
...
&НаКлиенте 
Процедура ОткрытьКаталогФайлаЗавершение(ДопПараметры) Экспорт 
	Если НЕ ТипЗнч(ДопПараметры) = Тип("Структура") Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте 
Процедура ОбновитьПиктограммуЗагрузкиСтраницыНаКлиенте(знач Завершено = Ложь)
...
Процедура ОбновитьДеревоСтраницНаКлиенте()
	...
	ДанныеПространства = ПолучитьДанныеСтраницыПространства(ЭтотОбъект);
	Если ДанныеПространства = Неопределено Тогда 
	...
	ЗаполнитьПодчиненныеСтраницыДереваНаКлиенте(ЭтотОбъект.ДеревоСтраниц, ДанныеПространства, ДоступныеСтраницы);
	
КонецПроцедуры

&НаКлиенте 
Процедура ЗаполнитьПодчиненныеСтраницыДереваНаКлиенте(знач ЭлементРодитель, знач СтрокаРодитель, знач ДоступныеСтраницы = Неопределено)
	...
	РезультатЗапроса = ПолучитьСтраницыПространстваНаКлиенте(СтрокаРодитель.СтраницаИдентификатор);
	Если РезультатЗапроса = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ТипЗнч(ЭлементРодитель) = Тип("ДанныеФормыДерево") Тогда
		СтрокаРаздела = ЭлементРодитель.ПолучитьЭлементы().Добавить();
		СтрокаРаздела.Идентификатор			= СтрокаРодитель.СтраницаИдентификатор;
		СтрокаРаздела.Заголовок				= НСтр("ru='Страница раздела'");
		СтрокаРаздела.СсылкаПросмотр		= СтрокаРодитель.СсылкаПросмотр;
		СтрокаРаздела.ЕстьПодчиненные		= Ложь;
		СтрокаРаздела.ПодчиненныеЗаполнены	= Ложь;
		СтрокаРаздела.КартинкаИзбранное		= ПолучитьИндексПиктограммыИзбранное(ЭтотОбъект, СтрокаРаздела.Идентификатор);
	КонецЕсли;
...
&НаКлиенте 
Функция ПолучитьОповещениеОткрытьКаталогФайла(знач ПутьКФайлу)
	Возврат Новый ОписаниеОповещения("ОткрытьКаталогФайлаЗавершение", ЭтотОбъект, Новый Структура("ПутьКФайлу", ПутьКФайлу));
КонецФункции
...
&НаКлиентеНаСервереБезКонтекста 
Функция ПолучитьДанныеСтраницыПространства(знач Форма)
	СтруктураОтбора = Новый Структура("Ключ", Форма.Объект.КлючПространства);
	СтрокиПространства = Форма.Пространства.НайтиСтроки(СтруктураОтбора);
	Если СтрокиПространства.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ДанныеПространства = Новый Структура("Идентификатор,Ключ,Наименование,Описание,СсылкаПросмотр,СтраницаИдентификатор,АнонимныйДоступ");
	ЗаполнитьЗначенияСвойств(ДанныеПространства, СтрокиПространства[0]);
...
Процедура ЗаполнитьНастройкиИзХранилищаНаСервере()
	...
	ЭтотОбъект.ЭтоАнонимныйДоступ = НЕ ЗначениеЗаполнено(Настройки.ПользовательЛогин);
	ЭтотОбъект.ЗначениеВРеквизитФормы(ОбъектФормы, "Объект");
	
	УстановитьУсловноеОформление(ЭтотОбъект, "РеквизитыАнонимныйДоступ");
КонецПроцедуры
...
&НаКлиенте 
Асинх Процедура ОбработатьКартинкиСтраницыНаКлиенте()
	
	Если НЕ ЭтотОбъект.ЭтоАнонимныйДоступ Тогда
		#Если ВебКлиент Тогда
			Настройки = ПолучитьНастройкиПодключения(Объект);
		    ОбработатьКартинкиСтраницыНаСервере(Настройки, ЭтотОбъект.ПолеHTML);
		#Иначе
			Ждать ОбработатьКартинкиСтраницыАсинх();
		#КонецЕсли		
	КонецЕсли;
...
&НаКлиенте 
Асинх Функция ОбработатьКартинкиСтраницыАсинх()
	ТекстHTML = ЭтотОбъект.ПолеHTML;
	ДокументHTML = ПолучитьДокументHTML(ТекстHTML);
	...
	ВыводитьСостояние = Элементы.ФормаВыводитьСостояниеЗагрузкиКартинок.Пометка;
	
	// закэшируем данные картинок на всякий случай
	КэшКартинок = Новый Соответствие;
	
	// скачаем и поместим картинки во временное хранилище
	Количество	= ДокументHTML.Картинки.Количество();
	Счетчик		= 0;
	Процент		= 0;
	...
	Для Каждого ЭлементКартинкаHTML Из ДокументHTML.Картинки Цикл
		Счетчик = Счетчик + 1;
		НовыйПроцент = Цел(Счетчик / Количество * 100);
		Если НовыйПроцент <> Процент И ВыводитьСостояние Тогда
			Процент = НовыйПроцент;
			Состояние(НСтр("ru='Обработка картинок'"), Процент,, БиблиотекаКартинок.confluence_Ожидание16);
		КонецЕсли;
		...
		ДанныеКартинки = ПолучитьДанныеКартинки(ЭлементКартинкаHTML.Атрибуты);
		
		КлючКартинки	= СтрШаблон("%1_%2", ДанныеКартинки.Страница, ДанныеКартинки.Картинка);
		АдресКартинки	= КэшКартинок.Получить(КлючКартинки);
		Если АдресКартинки = Неопределено Тогда
			АдресКартинки = Ждать confluence_ApiClientAsync.get_attachment_publiclink(Настройки,
				ДанныеКартинки.Страница,
				"att" + ДанныеКартинки.Картинка);
			
			Если НЕ ТипЗнч(АдресКартинки) = Тип("Строка") Тогда
				АдресКартинки = "";
			КонецЕсли;
			
			КэшКартинок.Вставить(КлючКартинки, АдресКартинки);
		КонецЕсли;
		
		СтильКартинки	= "";
		СтильКартинки	= СтильКартинки + ?(НЕ ЗначениеЗаполнено(ДанныеКартинки.Ширина), "", СтрШаблон("width:%1;", ДанныеКартинки.Ширина));
		СтильКартинки	= СтильКартинки + ?(НЕ ЗначениеЗаполнено(ДанныеКартинки.Высота), "", СтрШаблон("height:%1", ДанныеКартинки.Высота));
		
		ЭлементКартинкаHTML.УстановитьАтрибут("src", АдресКартинки);
		ЭлементКартинкаHTML.УстановитьАтрибут("style", СтильКартинки);
		ЭлементКартинкаHTML.УдалитьАтрибут("srcset");
	...
	ЭтотОбъект.ПолеHTML = ПолучитьТекстИзДокументаHTML(ДокументHTML); 
	
	Возврат Истина;
КонецФункции

&НаКлиентеНаСервереБезКонтекста 
Функция ОбработатьКартинкиСтраницыСлужебный(знач Настройки, знач ТекстHTML)
	ДокументHTML = ПолучитьДокументHTML(ТекстHTML);
	Если ДокументHTML = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Для Каждого ЭлементКартинкаHTML Из ДокументHTML.Картинки Цикл
		...
		АдресКартинки	= КэшКартинок.Получить(КлючКартинки);
		Если АдресКартинки = Неопределено Тогда
			АдресКартинки = confluence_ApiClientServer.get_attachment_publiclink(Настройки,
				ДанныеКартинки.Страница,
				"att" + ДанныеКартинки.Картинка);
		...
		СтильКартинки	= "";
		СтильКартинки	= СтильКартинки + ?(НЕ ЗначениеЗаполнено(ДанныеКартинки.Ширина), "", СтрШаблон("width:%1;", ДанныеКартинки.Ширина));
		СтильКартинки	= СтильКартинки + ?(НЕ ЗначениеЗаполнено(ДанныеКартинки.Высота), "", СтрШаблон("height:%2", ДанныеКартинки.Высота));
	...
	Возврат ПолучитьТекстИзДокументаHTML(ДокументHTML);
КонецФункции

&НаКлиентеНаСервереБезКонтекста 
Функция ПолучитьДокументHTML(знач ТекстHTML)
	Если СтрНайти(ТекстHTML, "<img") = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста 
Функция ПолучитьДанныеКартинки(знач Атрибуты)
	
	Результат = Новый Структура("Картинка,Страница,ТипДанных,Класс,Ширина,Высота");
	
	Результат.Картинка	= ПолучитьЗначениеАтрибутаКартинки(Атрибуты, "data-linked-resource-id");
	Результат.Страница	= ПолучитьЗначениеАтрибутаКартинки(Атрибуты, "data-linked-resource-container-id");
	Результат.ТипДанных	= ПолучитьЗначениеАтрибутаКартинки(Атрибуты, "data-linked-resource-content-type");
	Результат.Класс		= ПолучитьЗначениеАтрибутаКартинки(Атрибуты, "class");
		
	ШиринаКартинки = ПолучитьЗначениеАтрибутаКартинки(Атрибуты, "width");
	Если ЗначениеЗаполнено(ШиринаКартинки) Тогда
		Результат.Ширина = ШиринаКартинки;
	Иначе 
		Результат.Ширина = ПолучитьЗначениеАтрибутаКартинки(Атрибуты, "data-width");
		Результат.Высота = ПолучитьЗначениеАтрибутаКартинки(Атрибуты, "data-height");
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста 
Функция ПолучитьТекстИзДокументаHTML(знач ДокументHTML)
...
Процедура ОбработатьКартинкиСтраницыНаСервере(знач Настройки, ТекстHTML)
	Результат = ОбработатьКартинкиСтраницыСлужебный(Настройки, ТекстHTML);
	Если ПустаяСтрока(Результат) Тогда
		Возврат;
	КонецЕсли;
	
	ЭтотОбъект.ПолеHTML = Результат;
КонецПроцедуры
...
Функция ПолучитьСодержимоеТекущейСтраницыНаКлиенте()
	...
	ПараметрыКонтента.Контент.metadata_labels	= Истина;
	...
	Для Каждого СтруктураМетки Из СтруктураКонтента.Метки.Значения Цикл
		СтрокаМетки = ЭтотОбъект.Метки.Добавить();
...
Процедура ВыполнитьПереходНаСтраницуРаздела()
	ДанныеПространства = ПолучитьДанныеСтраницыПространства(ЭтотОбъект);
	Если НЕ ДанныеПространства = Неопределено Тогда
		ВыполнитьПереходНаСтраницу(ДанныеПространства.СтраницаИдентификатор, ДанныеПространства.Наименование);
	КонецЕсли;
КонецПроцедуры
...
Процедура ВыполнитьПереходНаСтраницу(знач ИдентификаторСтраницы, знач Заголовок)
	...
	ОбновитьПиктограммуЗагрузкиСтраницыНаКлиенте();
...
Процедура ВыполнитьПереходНаСтраницуОбработчик()
	ЭтотОбъект.КартинкиОбработаны = Ложь;
...
#Область СлужебныеПроцедурыИФункции_ЭкспортПространства

&НаКлиенте 
Процедура ЭкспортПространстваВXMLНаКлиенте(знач ПутьКФайлу)
	НастройкиПодключения = ПолучитьНастройкиПодключения(Объект);
	
	Результат = confluence_ApiClientServer.get_space_as_xml(НастройкиПодключения,
		Объект.КлючПространства);
	...
	Оповещение = ПолучитьОповещениеОткрытьКаталогФайла(ПутьКФайлу);
	
	ПоказатьОповещениеПользователя(НСтр("ru='Запись выполнена'"),
		Оповещение,
		НСтр("ru='Файл ZIP сохранен. Нажмите для открытия каталога'"),
		БиблиотекаКартинок.Информация);
КонецПроцедуры

&НаКлиенте 
Процедура ЭкспортПространстваPDFНаКлиенте(знач ПутьКФайлу)
	НастройкиПодключения = ПолучитьНастройкиПодключения(Объект);
	
	Результат = confluence_ApiClientServer.get_space_as_pdf(НастройкиПодключения,
		Объект.КлючПространства);
