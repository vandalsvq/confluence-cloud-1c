...
Асинх Процедура СохранитьКакPDF(Команда)
	...
	Результат = confluence_ApiClientServer.get_page_as_pdf(НастройкиПодключения, ЭтотОбъект.ТекущаяСтраницаИдентификатор);
...
Функция ПолучитьДанныеСсылкиПоЭлементу(знач Anchor)
	...
	Если СтрНачинаетсяС(Структура.pathname, "/wiki/spaces/") Тогда
		Разделитель = СтрНайти(Структура.pathname, "/pages/", НаправлениеПоиска.СКонца);
		Если Разделитель > 0 Тогда
			АдресСтраницы = Сред(Структура.pathname, Разделитель + 7);
			
			Если НЕ СтрНайти(АдресСтраницы, "/") Тогда
				Структура.linkedResourceId		= АдресСтраницы;
